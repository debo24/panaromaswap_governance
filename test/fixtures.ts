...
export async function governanceFixture(
  ...
  // deploy bang, sending the total supply to the deployer
  const { timestamp: now } = await provider.getBlock('latest')
  const timelockAddress = Contract.getContractAddress({ from: wallet.address, nonce: 1 })
  const bang = await deployContract(wallet, Bang, [wallet.address, timelockAddress, now + 60 * 60])
  ...
  const governorAlpha = await deployContract(wallet, GovernorAlpha, [timelock.address, bang.address])
  expect(governorAlpha.address).to.be.eq(governorAlphaAddress)

  return { bang, timelock, governorAlpha }
}
