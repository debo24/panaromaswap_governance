...
import BangswapV2Factory from 'panaromaswap_v2core/build/PanaromaswapV2Factory.json'
import BangswapV2Pair from 'panaromaswap_v2core/build/PanaromaswapV2Pair.json'
...
describe('scenario:FeeTo', () => {
  ...
  beforeEach('deploy Bangswap v2', async () => {
    factory = await deployContract(wallet, BangswapV2Factory, [wallet.address])
  })
  ...
  describe('tokens', () => {
    ...
    beforeEach('make test tokens', async () => {
      const { timestamp: now } = await provider.getBlock('latest')
      const token0 = await deployContract(wallet, Bang, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token0)
      const token1 = await deployContract(wallet, Bang, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token1)
    })
    ...
    beforeEach('create fee liquidity', async () => {
      ...
      pair = new Contract(pairAddress, BangswapV2Pair.abi).connect(wallet)
