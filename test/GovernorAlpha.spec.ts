...
describe('GovernorAlpha', () => {
  ...
  it('bang', async () => {
    const balance = await bang.balanceOf(wallet.address)
    const totalSupply = await bang.totalSupply()
  ...
  it('governor', async () => {
    ...
    const bangFromGovernor = await governorAlpha.bang()
    expect(bangFromGovernor).to.be.eq(bang.address)
  })
})
